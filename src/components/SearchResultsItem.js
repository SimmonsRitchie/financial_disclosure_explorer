...
const SearchResultsItem = props => {
  ...
  // one part is cut off.
  const snippetReplace = reactStringReplace(
    snippetEsc,
    /b4de2a49c8(.*)8c94a2ed4b/gi,
    (match, i) => (
      <span key={i} style={{ color: "red" }}>
        {match.replace(/b4de2a49c8|8c94a2ed4b/gi, "")}
      </span>
    )
  );
      ...
      <div className="results-item__container-header" onClick={handleClick}>
        <div>
          <div className="has-text-weight-bold is-size-5">
            {q01_first_name} {q01_middle_name && `${q01_middle_name[0]}.`}{" "}
            {q01_last_name}
          </div>
          <div className="is-size-6">
            Filing year: {q07_filing_year}, Filing as: {q03_status},{" "}
            {q04_state_or_local}
          </div>
          {highlight && (
            <div className="results-item__preview">
              <span className="is-size-6 is-italic">...{highlight}...</span>
            </div>
          )}
        </div>
        <div>{expandText}</div>
